...
const RecetasScreen: React.FC = () => {
  ...
  React.useEffect(() => {
    if (
      productoRecetaEditar &&
      productos.find((p) => p.id === productoRecetaEditar)
    ) {
  ...
  const handleAgregarIngrediente = (ingredienteId: string) => {
    if (
      !ingredientesSeleccionados.find(
  ...
  const ganancia = productoActual
    ? productoActual.precioVenta - costoProduccion
    : 0;
  const margenGanancia = productoActual
    ? ((ganancia / productoActual.precioVenta) * 100).toFixed(2)
    : 0;
              ...
              <Text
                style={[
                  ...
                  productoSeleccionado === p.id &&
                    styles.productoChipTextSelected,
                ]}
              >
                {p.nombre}
              </Text>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.infoRow}>
            ...
            <Text style={styles.infoValue}>
              ${productoActual.precioVenta.toFixed(2)}
            </Text>
          </View>
          ...
          <TouchableOpacity
            style={styles.buttonEditarReceta}
            onPress={abrirEditarReceta}
          >
                ...
                return (
                  <View key={item.ingredienteId} style={styles.ingredienteRow}>
                    ...
                    <Text style={styles.ingredienteCantidad}>
                      {item.cantidad}{' '}
                      {getUnidadLabel(ingrediente?.tipoUnidad || '')}
                    </Text>
                    <Text style={styles.ingredienteCosto}>
                      ${(
                        ingrediente
                          ? ingrediente.precioUnitario * item.cantidad
                          : 0
                      ).toFixed(2)}
                    </Text>
                  </View>
              ...
              {ingredientesSeleccionados.map((item) => {
                ...
                return (
                  <View
                    ...
                    style={styles.ingredienteEditRow}
                  >
                    <View style={styles.ingredienteEditInfo}>
                      <Text style={styles.ingredienteEditNombre}>
                        {ingrediente?.nombre}
                      </Text>
                    ...
                    <TextInput
                      ...
                      placeholder="0"
                    />
                    <Text style={styles.ingredienteEditUnidad}>
                      {getUnidadLabel(ingrediente?.tipoUnidad || '')}
                    </Text>
                    <TouchableOpacity
                      style={styles.ingredienteDeleteButton}
                      onPress={() =>
                        handleEliminarIngrediente(item.ingredienteId)
                      }
                    >
            ...
            <View style={styles.agregarIngredienteContainer}>
              {ingredientes
                ...
                .map((i) => (
                  ...
                  >
                    <Text style={styles.agregarIngredienteButtonText}>
                      + {i.nombre}
                    </Text>
                  </TouchableOpacity>
                ))}
...
const styles = StyleSheet.create({
  ...
  ingredienteEditNombre: {
    fontSize: 13,
    fontWeight: '600',
    color: '#111827',
  },
  ingredienteEditPrecio: {
    fontSize: 11,
    color: '#6b7280',
    marginTop: 2,
  },
